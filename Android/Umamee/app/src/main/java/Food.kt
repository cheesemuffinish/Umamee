...
open class Food {
    ...
    fun remove(amount: Double, unitsGiven: String){
        ...
        if (this.amount == null){
            ...
            this.amount = 0.0
            println("You are audi 5000 of ${this.uniqueName}")
        }
    }

    fun convertUnits(amountIn: Double, unitsGiven: String, unitsDesired: String): Double? {

        val massConversionNames = listOf<String>("grams","kilograms", "ounces", "pounds")

        val massConversionValues = doubleArrayOf(1000.0, 1.0, 35.274, 2.20462)

        val volumeConversionNames = listOf<String>("teaspoons", "tablespoons", "fluid ounces", "cups",
            "pints", "quarts", "gallons", "milliliters", "liters")

        val volumeConversionValues = doubleArrayOf(48.0, 16.0, 8.0, 1.0, 0.5, 0.25, 0.0625, 236.588, 0.236588)
        ...
        }else if(unitsGiven == "items"){
            if(unitsDesired == "items"){
                return amountIn
            }else{
                println("The units given are incompatible with ${this.units}")
            }
